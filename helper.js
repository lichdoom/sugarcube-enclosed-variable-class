...
setup.defineProperties = function(source, target, limits = {}) {
	for (const key in source) {
		...
		if (val && type === 'object' && !Array.isArray(val)) {
			...
			setup.defineProperties(val, target[key], limits[key]);
			continue;
		}
...
setup.deepMerge = function(target, data) {
...
setup.deepFreeze = function(obj) {
	Object.freeze(obj);
	for (const key in obj) {
		const val = obj[key];
		if (val && typeof val === 'object') setup.deepFreeze(val);
	}
};
// Helper function 
setup.validateLimits = function(obj, limits, path = '', errors = []) {
	for (const key in limits) {
		...
		if (val && typeof val === 'object' && !Array.isArray(val)) {
			if (limit && typeof limit === 'object' && !Array.isArray(limit)) {
				setup.validateLimits(val, limit, path + key + '.', errors);
			} else if (limit) {
